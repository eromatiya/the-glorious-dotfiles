...
local set_markup = function(mail_count)
  ...
  if mail_count == 'no-credentials' then
    email_count.markup = '<span font="SFNS Display Regular 12">Credentials are required!</span>'
    return
  end

  if mail_count == 'no-network' then
    email_count.markup = '<span font="SFNS Display Regular 12">Check internet connection!</span>'
    return
  end


  if tonumber(mail_count) <= 1 then
    email_count.markup = '<span font="SFNS Display Regular 12">You have ' .. mail_count .. ' unread email!</span>'
  else
    email_count.markup = '<span font="SFNS Display Regular 12">You have ' .. mail_count .. ' unread emails!</span>'
  end
end
